...
const getIntensityColor = (intensity) => {
    ...
    if (intensity < 200) return '#8bc34a';
    if (intensity < 300) return '#ffeb3b';
    if (intensity < 400) return '#ff9800';
    return '#f44336';
};
...
function Dashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setError('Failed to fetch data. Is the backend running on http://localhost:5000?');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const avgIntensity = chartData.length > 0
        ? chartData.reduce((acc, d) => acc + d.intensity, 0) / chartData.length
        : 0;
    ...
    return (
        <Box>
                        ...
                        <CardContent>
                            ...
                            {bestDC ? (
                                <>
                                    ...
                                    <Typography variant="body2">{bestDC.intensity.toFixed(0)} gCO₂/kWh</Typography>
                                </>
                            ) : (
                        ...
                        <CardContent>
                            ...
                            {worstDC ? (
                                <>
                                    ...
                                    <Typography variant="body2">{worstDC.intensity.toFixed(0)} gCO₂/kWh</Typography>
                                </>
                            ) : (
                                <Typography variant="body2">Loading...</Typography>
                            )}
                    ...
                    <Card sx={{ borderTop: 3, borderColor: 'primary.main' }}>
                        <CardContent>
                            ...
                            <Typography variant="h5" fontWeight="bold">
                                {avgIntensity.toFixed(0)} <Typography component="span" variant="body2">gCO₂/kWh</Typography>
                            </Typography>
                        </CardContent>
                    </Card>
                </Grid>
                            ...
                            <Box height={350}>
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={sortedChartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                        ...
                                        <Tooltip
                                            formatter={(value) => [`${value.toFixed(1)} gCO₂/kWh`, 'Carbon Intensity']}
                        ...
                        <CardContent>
                            ...
                            <Box sx={{ maxHeight: 350, overflow: 'auto' }}>
                                {datacenters.map((dc) => {
                                    ...
                                    const intensity = carbon.intensity || 0;
                                    const renewable = carbon.renewable || 0;
                                    return (
                                        <Box
                                            key={dc.id}
                                            sx={{
                                        ...
                                        >
                                            <Box display="flex" justifyContent="space-between" alignItems="center">
                                                ...
                                                <Box textAlign="right">
                                                    <Typography variant="subtitle2" fontWeight="bold">{intensity.toFixed(0)}</Typography>
                                                    <Chip
                                                        label={getIntensityLabel(intensity)}
                                                        size="small"
                                                        sx={{
                                                            bgcolor: getIntensityColor(intensity),
                                                            color: intensity < 300 ? 'black' : 'white',
                                                            fontSize: '0.7rem',
                                                            height: 20,
                                                        }}
                                                    />
                                            ...
                                            <Box mt={1}>
                                                <Box display="flex" alignItems="center" gap={0.5}>
                                                    ...
                                                    <Typography variant="caption">{renewable.toFixed(0)}% Renewable</Typography>
                                                </Box>
                                                <LinearProgress
                                                    variant="determinate"
            ...
            <Box mt={3} display="flex" gap={2}>
                <Button variant="contained" size="large" onClick={() => navigate('/optimize')} startIcon={<CloudIcon />}>
                    Optimize Workload
                </Button>
                <Button variant="outlined" size="large" onClick={() => navigate('/compare')} startIcon={<CompareIcon />}>
                    Compare Algorithms
                </Button>
            </Box>
        </Box>
